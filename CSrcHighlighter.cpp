...
CSrcHighlighter::CSrcHighlighter(std::vector<std::string>& file) noexcept : file{file}, unmodifiedFile{file}
{

}


void CSrcHighlighter::importKeywords()
{
...
void CSrcHighlighter::importPreprocessorDirectives()
{
...
void CSrcHighlighter::doSyntaxHighlighting()
{
...
void CSrcHighlighter::exportHighlightedSrcCode(const std::string& str)
{
    importKeywords();
...
void CSrcHighlighter::highlightCharsAndStringLiterals()
{
    for (auto& line : file)
    {
...
void CSrcHighlighter::highlightEscapes()
{
    for (auto& line : file)
    {
...
void CSrcHighlighter::highlightKeywords()
{
...
void CSrcHighlighter::highlightPreprocessorDirectives()
{
    for (auto& line : file)
    {
        for (const auto& prep_dir : prep_dirs)
        {
            std::stringstream prep_regexSS;
            prep_regexSS << "[ \t]*#" << prep_dir << "[ \t]+.*|[ \t]*#" << prep_dir;

            if (std::regex_match(line, std::regex(prep_regexSS.str())))
            {
                if (std::regex_search(line, std::regex("<[[:alpha:]]+(.(h|(hpp)))?>")))
                {
...
void CSrcHighlighter::highlightNumericConstants()
{
    for(int x = 0; x < static_cast<int>(file.size()); ++x)
    {
        auto& line = file[x];
        auto& unmodifiedLine = unmodifiedFile[x];

        std::regex regexForStringEncapsulatedNums(R"("[^"]*[[:digit:]]+[^"]*")");
        if (!std::regex_search(unmodifiedLine, regexForStringEncapsulatedNums))
        {
            std::regex regexForNumericConst("0(x|X)[a-fA-F0-9]+|^[[:digit:]]*\\.?[[:digit:]]+f?|^[[digit]]+|[ \t\\)\\(><=+-/\\\\[*;,{}][[:digit:]]*\\.?[[:digit:]]+f?|[ \t\\)\\(><=+-/\\\\[*;,{}][[digit]]+");
...
void CSrcHighlighter::handleSingleLineComments()
{
    for(auto& line : file)
    {
        ...
        if (commentLocation != std::string::npos)
        {
            ...
            // ------------------------------------------------------------------------^ escapelni kell pár karaktert,
            // mert szeretnénk regexként használni
...
void CSrcHighlighter::handleMultiLineComments()
{
...
void CSrcHighlighter::doubleCheckStringLiterals()
{
